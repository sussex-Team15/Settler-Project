...
screen = pygame.display.set_mode(
    (DISPLAY_WIDTH, DISPLAY_HEIGHT), pygame.DOUBLEBUF
)
...
def setup():
    ...
    for gametile in board:
        ...
        image = pygame.transform.scale(
            image,
            (image.get_rect().width / 2.5, image.get_rect().height / 2.5),
        )
...
class StartMenu:
    ...
    def draw(self, screen):
        ...
        background_img = pygame.image.load(
            os.path.join(
                "src", "assets", "images", "start-menu-background.jpg"
            )
        )
        ...
        welcome_text_surface = WELCOME_FONT.render(
            "Welcome to Catan!", True, BLACK
        )
        ...
        screen.blit(
            input_text_surface, (self.input_box.x + 5, self.input_box.y + 5)
        )

        for i, player in enumerate(players):
            player_text = f"Player {i+1}: {player.name}"
            text_surface = WORD_FONT.render(
                player_text, True, WHITE, player.color
            )
            text_rect = text_surface.get_rect(
                center=(DISPLAY_WIDTH - 150, 500 + i * 50)
            )
        ...
        screen.blit(
            ...
            (self.start_button_rect.x, self.start_button_rect.y),
        )

        pygame.draw.rect(screen, WHITE, self.rule_book_button_rect)
        screen.blit(
            self.rule_book_button_text,
            (self.rule_book_button_rect.x, self.rule_book_button_rect.y),
        )

        # draw checkbox for AI player

        checkbox_prompt_text = WORD_FONT.render(
            "Check box to set as AI player", True, BLACK
        )
        checkbox_prompt_rect = checkbox_prompt_text.get_rect(
            center=(700, DISPLAY_HEIGHT // 2 + 150)
        )
        screen.blit(checkbox_prompt_text, checkbox_prompt_rect)

        input_prompt_text = WORD_FONT.render("Enter name here: ", True, BLACK)
        input_prompt_rect = input_prompt_text.get_rect(
            center=(DISPLAY_WIDTH // 2 - 320, DISPLAY_HEIGHT // 2 + 70)
        )
        ...
        if self.checked:
            pygame.draw.line(
                ...
                (self.checkbox_rect.left + 10, self.checkbox_rect.bottom - 5),
                4,
            )
            pygame.draw.line(
                screen,
...
class SpecialRoundGameState:  # gamestate for the first 2 turns of the game (players can build 1 road and settlement free)
    ...
    def __init__(self, player):
        ...
        self.settlement_img = pygame.image.load(
            os.path.join("src", "assets", "buildings", "settlement.png")
        )
    ...
    def handle_events(self, events):
        ...
        for _node_id, node_point in board_mapping["nodes"].items():
            button = ButtonHex((node_point[0], node_point[1]), 10, GRAY)
            self.node_buttons.append(button)

        for event in events:
            print(self.current_player.__class__.__name__)
            ...
            if self.current_player.__class__.__name__ is not "AIPlayer":
                ...
                if event.type == pygame.MOUSEBUTTONDOWN:
                    for node_button in self.node_buttons:
                        if node_button.is_clicked(mouse_pos):
                            if self.settlement_node1 == None:
                                ...
                                self.current_player.build_settlement(
                                    self.settlement_node1,
                                    is_special_round=True,
                                )
                                built_settlements.append(
                                    (
                                        self.settlement_node1,
                                        self.current_player,
                                    )
                                ...
                                if self.road_1[0] is None:
                                    ...
                                    1
                                ] is None and self.is_adjacent(
                                    self.road_1[0], node_button
                                ):
                                    self.road_1[1] = node_button
                                    self.current_player.build_road(
                                        self.road_1[0],
                                        self.road_1[1],
                                        is_special_round=True,
                                    )
                    ...
                    if event.key == pygame.K_SPACE:
                        if self.current_turn_number == len(players) - 1:
                            self.current_state = MainGameState(
                                self.current_player
                            )
                        ...
                        self.current_player.settlements.append(
                            self.settlement_node1
                        )
                        self.current_player.settlements.append(
                            self.settlement_node2
                ...
                if self.settlement_node1 == None:
                    ...
                    self.current_player.build_settlement(
                        self.settlement_node1, is_special_round=True
                    )
                    built_settlements.append(
                        (self.settlement_node1, self.current_player)
                    )
                    return
                if self.settlement_node2 == None:
                    ...
                    self.current_player.build_settlement(
                        self.settlement_node2, is_special_round=True
                    )
                    built_settlements.append(
                        (self.settlement_node2, self.current_player)
                    )
                    return

                # Build 2 Roads
                print(adjacent_nodes)
                inverse_board_mapping = {
                    v: k for k, v in board_mapping["nodes"].items()
                }
                settlement_node_1_coords = (
                    self.settlement_node1.x_pos,
                    self.settlement_node1.y_pos,
                )
                settlement_node_2_coords = (
                    self.settlement_node1.x_pos,
                    self.settlement_node1.y_pos,
                )
                settlement_node_1_id = inverse_board_mapping[
                    settlement_node_1_coords
                ]
                settlement_node_2_id = inverse_board_mapping[
                    settlement_node_2_coords
                ]
                ...
                if self.road_1[0] is None and self.road_1[1] is None:
                    ...
                    self.road_1[0] = ButtonHex(
                        board_mapping["nodes"][road_choice[0]], 10, GRAY
                    )
                    self.road_1[1] = ButtonHex(
                        board_mapping["nodes"][road_choice[1]], 10, GRAY
                    )

                    self.current_player.build_road(
                        self.road_1[0], self.road_1[1], is_special_round=True
                    )
                    built_roads.append(
                        ((self.road_1[0], self.road_1[1], self.current_player))
                    )
                if self.road_2[0] is None and self.road_2[1] is None:
                    ...
                    road_choice = random.choice(road_choices)
                    self.road_2[0] = ButtonHex(
                        board_mapping["nodes"][road_choice[0]], 10, GRAY
                    )
                    self.road_2[1] = ButtonHex(
                        board_mapping["nodes"][road_choice[1]], 10, GRAY
                    )

                    self.current_player.build_road(
                        self.road_2[0], self.road_2[1], is_special_round=True
                    )
                    built_roads.append(
                        ((self.road_2[0], self.road_2[1], self.current_player))
                    )
    ...
    def draw(self, screen):
        ...
        prompt_text = WORD_FONT.render(
            f"{self.current_player.name}: build two settlements and two roads",
            True,
            BLACK,
        )
        ...
        for settlement in built_settlements:
            ...
            highlighted_img = pygame.Surface(
                settlement_img.get_size(), pygame.SRCALPHA
            )
            highlighted_img.fill(settlement[1].color)
            highlighted_img.blit(
                settlement_img, (0, 0), None, pygame.BLEND_RGBA_MULT
            )

            screen.blit(
                highlighted_img,
                (settlement[0].x_pos - 25, settlement[0].y_pos - 25),
            )
    ...
    def is_adjacent(self, node1, node2):
        ...
        for settlement in built_settlements:
            node = settlement[0]
            if (node.x_pos == node1.x_pos and node.y_pos == node1.y_pos) or (
                node.x_pos == node2.x_pos and node.y_pos == node2.y_pos
            ):
        ...
        return (
            (x_diff <= max_road_len)
            and (y_diff <= max_road_len)
            and adjacent_to_building
        )
    ...
    def draw_lines(self):
        ...
        for tile in board:
            ...
            pygame.draw.line(
                ...
                "white",
                board_mapping["nodes"][tile.node_coord_n],
                board_mapping["nodes"][tile.node_coord_ne],
                5,
            )
...
class MainGameState:
            ...
            if self.current_player.__class__.__name__ is not "AIPlayer":
                if event.type == pygame.MOUSEBUTTONDOWN:
                    ...
                    if event.key == pygame.K_SPACE:
                        ...
                        dice_roll1, dice_roll2 = self.current_player.roll_dice(
                            2
                        )
                        ...
                        for game_tile in board:
                            if (
                                dice_roll1 + dice_roll2
                                == game_tile.real_number
                            ):
                                self.current_player.add_resource(
                                    game_tile.tile.generate_resource().name()
                                )
                        ...
                        if (
                            dice_roll1 + dice_roll2 == 7
                        ):  # player can add robber to tile
                            ...
                            game_log.append(game_log_txt)
                            self.current_state = PlaceRobberState(
                                self.current_player
                            )
            else:
                choices = ["build settlement", "build road"]
                ...
                if choice == "build settlement":
                    ...
                    if self.settlement_node1 == None:
                        self.settlement_node1 = random.choice(
                            settlement_choices
                        )
                        settlement_choices.remove(self.settlement_node1)
                        self.current_player.build_settlement(
    ...
    def draw(self, screen):
        ...
        robber_img = pygame.image.load(
            os.path.join("src", "assets", "Tiles", "robber.jpg")
        )
        robber_img = pygame.transform.scale(
            robber_img,
            (robber_img.get_width() * 0.05, robber_img.get_height() * 0.05),
        )
        ...
        for gametile in board:
            if gametile.has_robber and not self.robber_coords is None:
                screen.blit(
                    robber_img,
                    (self.robber_coords[0] + 70, self.robber_coords[1] + 65),
                )
        ...
        for settlement in built_settlements:
            ...
            screen.blit(
                player_colors[settlement[1]],
                (settlement[0].x_pos - 25, settlement[0].y_pos - 25),
            )

        for city in built_cities:
            highlighted_img = pygame.Surface(
                city_img.get_size(), pygame.SRCALPHA
            )
            highlighted_img.fill(city[1].color)
            highlighted_img.blit(
                city_img, (0, 0), None, pygame.BLEND_RGBA_MULT
            )
            player_colors[city[1]] = highlighted_img
            screen.blit(
                player_colors[city[1]],
                (city[0].x_pos - 25, city[0].y_pos - 25),
            )
    ...
    def draw_scoreboard(self, player_turn):
        ...
        pygame.draw.rect(
            rect_surf, WHITE, scoreboard_outline, rect_outline_width
        )
        pygame.draw.line(rect_surf, WHITE, (10, 80), (rect_width - 10, 80), 6)
        pygame.draw.line(
            rect_surf, WHITE, (10, 160), (rect_width - 10, 160), 6
        )
        pygame.draw.line(
            rect_surf, WHITE, (10, 430), (rect_width - 10, 430), 6
        )  # space for gamelog

        player_width = scoreboard_width // NUM_PLAYERS
        font_size = (
            20 if len(players) == 6 else 30 if len(players) == 5 else 40
        )
        ...
        for i in range(len(players)):  # draw player names at top of scoreboard
            ...
            pygame.draw.line(
                rect_surf, WHITE, (x_pos, 10), (x_pos, 430), rect_outline_width
            )
        ...
        for i in range(len(players)):  # draw victory points
            ...
            pygame.draw.line(
                rect_surf, WHITE, (10, 510), (rect_width - 10, 510), 6
            )
            name = player_font.render(
                f"VP: {players[i].victory_points}", True, WHITE
            )
        ...
        player_turn_text = WORD_FONT.render(
            f"Current turn: {player_turn.name}", True, WHITE
        )
    ...
    def draw_buttons(self):
        ...
        for _node_id, node_point in board_mapping["tiles"].items():
            button = ButtonHex(
                (node_point[0], node_point[1]), button_radius[1], WHITE, False
            )
            tile_buttons.append(button)
            # invisible buttons at center of tiles

        build_road_text = WORD_FONT.render(
            "Build Road", True, WHITE, (17, 104, 245)
        )
        screen.blit(build_road_text, self.build_road_rect)

        build_settlement_text = WORD_FONT.render(
            "Build Settlement", True, WHITE, (38, 140, 31)
        )
        screen.blit(build_settlement_text, self.build_settlement_rect)

        build_city_text = WORD_FONT.render(
            "Build City", True, WHITE, (181, 186, 43)
        )
        screen.blit(build_city_text, self.build_city_rect)

        make_trade_text = WORD_FONT.render(
            "Trade", True, WHITE, (255, 51, 153)
        )
        screen.blit(make_trade_text, self.make_trade_rect)

        dev_card_text = WORD_FONT.render(
            "Development Card", True, WHITE, (51, 153, 255)
        )
        screen.blit(dev_card_text, self.dev_card_rect)

        bank_inventory_text = WORD_FONT.render(
            "Bank", True, WHITE, (255, 153, 51)
        )
...
class InventoryGameState:
    ...
    def draw(self, screen):
        ...
        images.append(
            pygame.image.load(
                os.path.join("src", "assets", "resource", "lumber.jpg")
            )
        )
        images.append(
            pygame.image.load(
                os.path.join("src", "assets", "resource", "wool.jpg")
            )
        )
        images.append(
            pygame.image.load(
                os.path.join("src", "assets", "resource", "Grain.jpg")
            )
        )
        images.append(
            pygame.image.load(
                os.path.join("src", "assets", "resource", "brick.jpg")
            )
        )
        images.append(
            pygame.image.load(
                os.path.join("src", "assets", "resource", "ore.jpg")
            )
        )

        x_offset_image = 20
        x_offset__text = 50
        for i, (resource, quantity) in enumerate(
            self.player.resources.items()
        ):
            if resource is None:
                continue
            ...
            resource_info_text = WORD_FONT.render(
                f"{resource} : {quantity}", True, BLACK
            )
            resource_info_rect = resource_info_text.get_rect(
                center=(x_offset__text, 650)
            )
        ...
        screen.blit(
            BIG_FONT.render("Your Current Resources", True, BLACK), (200, 50)
        )
        screen.blit(
            BIG_FONT.render("Back", True, BLACK, RED), self.back_button
        )
...
class PlaceRobberState:
    ...
    def __init__(self, player):
        ...
        self.player = player
        self.current_state = None
        self.robber_img = pygame.image.load(
            os.path.join("src", "assets", "Tiles", "robber.jpg")
        )
                    ...
                    if target_rect.collidepoint(mouse_pos):
                        for game_tile in board:
                            if game_tile.get_tile_info()["Tile id"] == id:
                                ...
                                game_log.append(
                                    f"{self.player.name} placed a robber"
                                )
                                self.current_state = MainGameState(
                                    self.player,
                                    (mouse_pos[0] - 60, mouse_pos[1] - 50),
                                )
    ...
    def draw(self, screen):
        ...
        """
        prompt_text = WORD_FONT.render(
            f"Place the robber on a tile", True, BLACK, RED
        )
...
class ChooseResources:
        ...
        for event in events:
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                for i, minus_button in enumerate(self.minus_buttons):
                    if (
                        minus_button.collidepoint(event.pos)
                        and self.offered_resources[i] > 0
                    ):
                        self.offered_resources[i] -= 1
                if self.back_button.collidepoint(event.pos):
                    self.current_state = MainGameState(self.player)
                    ...
                    if self.trade_partner is None:  # submit for bank trade
                        ...
                        resources_trade = dict(
                            zip(resource_names, self.offered_resources)
                        )
                        if (
                            self.has_enough_resources(
                                self.player.resources, resources_trade
                            )
                            and sum(resources_trade.values()) == 4
                        ):
                            self.current_state = BankTrade(
                                self.player, bank, resources_trade
                            )
                        else:
                            self.current_state = NotEnoughResources(
                                self.player
                            )
                    else:
                        resource_names = [
                        ...
                        if self.has_enough_resources(
                            ...
                            self.current_state = ChooseDesiredResources(
                                self.player,
                                self.trade_partner,
                                resources_trade,
                            )
    ...
    def draw(self, screen):
        ...
        for i in range(len(images)):
            ...
            plus_button_rect.centerx = (
                x_offset_image + images[i].get_width() / 2
            )
            ...
            quantity_text = WORD_FONT.render(
                str(self.offered_resources[i]), True, BLACK
            )
            quantity_rect = quantity_text.get_rect()
            quantity_rect.centerx = (
                x_offset_image + images[i].get_width() / 2 + 25
            )
            quantity_rect.y = 160
        ...
        for resource, quantity in self.player.resources.items():
            if resource == None:
                ...
                trade_quantity_text = WORD_FONT.render(
                    f"amount owned: {quantity}", True, BLACK
                )
                trade_quantity_rect = trade_quantity_text.get_rect(
                    center=(x_offset, 650)
                )
        ...
        screen.blit(
            BIG_FONT.render("Your Current Resources", True, BLACK), (200, 30)
        )
        screen.blit(
            BIG_FONT.render("Back", True, BLACK, RED), self.back_button
        )
        screen.blit(
            BIG_FONT.render("Submit Resources", True, BLACK),
            self.submit_button,
        )
    ...
    def has_enough_resources(self, player_resources, offered_resources):
        for (
            ...
            offered_resources.items()
        ):  # check to see if player has enough resources
            if (
                resource not in player_resources
                or player_resources[resource] < quantity
            ):
                return False
        return True
...
class ChooseDesiredResources:
    ...
    def __init__(self, player, trade_partner, offered_resources):
        ...
        self.resource_buttons = (
            []
        )  # The resource buttons of the trade partner the player desires
        self.back_button = pygame.Rect(20, 680, 200, 200)
        ...
        for event in events:
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                for i, minus_button in enumerate(self.minus_buttons):
                    if (
                        minus_button.collidepoint(event.pos)
                        and self.desired_resources[i] > 0
                    ):
                        self.desired_resources[i] -= 1
                if self.back_button.collidepoint(event.pos):
                    self.current_state = MainGameState(self.player)
                    ...
                    if self.trade_partner is None:  # submit for bank trade
                        ...
                        resources_trade = dict(
                            zip(resource_names, self.desired_resources)
                        )
                        if self.has_enough_resources(
                            self.trade_partner.resources, resources_trade
                        ):
                            self.current_state = BankTrade(
                                self.player, bank, resources_trade
                            )
                        ...
                        if self.has_enough_resources(
                            ...
                            self.current_state = AcceptTradeState(
                                self.player,
                                self.trade_partner,
    ...
    def draw(self, screen):
        ...
        for i in range(len(images)):
            ...
            # Draw resource quantity next to the + and - minus buttons
            quantity_text = WORD_FONT.render(
                str(self.desired_resources[i]), True, BLACK
            )
            quantity_rect = quantity_text.get_rect()
            quantity_rect.centerx = (
                x_offset_image + images[i].get_width() / 2 + 25
        ...
        screen.blit(
            BIG_FONT.render(
                f"{self.trade_partner.name}: Current Resources", True, BLACK
            ),
            (200, 30),
        )
        screen.blit(
...
class BankTrade:
    ...
    def draw(self, screen):
        ...
        for image, resource in resource_images:
            ...
            rect = pygame.Rect(
                x_offset, 100, image.get_width(), image.get_height()
            )
...
class DevelopmentCardState:
    def __init__(self, player):
        ...
        self.card_images = {
            DevelopmentCards.KNIGHT: (
                pygame.image.load(
                    os.path.join("src", "assets", "Development", "knight.jpg")
                ),
                pygame.Rect(300, 50, 100, 150),
            )
        }
    ...
    def draw(self, screen):
        ...
        for card, (image, rect) in self.card_images.items():
            ...
            rect = pygame.Rect(
                rect.x, rect.y, image.get_width(), image.get_height()
            )
        ...
        font_size = min(
            100, int(self.back_button_rect.height * 0.8)
        )  # sets font size to be 80% size of recatngle
...
class Build:
                    ...
                    if node_button.is_clicked(mouse_pos):
                        ...
                        if self.has_enough_resources():
                            if self.is_city:
                                ...
                                game_log.append(
                                    f"{self.player.name} built a city!"
                                )
                                self.current_state = MainGameState(self.player)
                            else:
                                ...
                                built_settlements.append(
                                    (self.node, self.player)
                                )
                                game_log.append(
                                    f"{self.player.name} built a settlement!"
                                )
                                self.current_state = MainGameState(self.player)
                        else:
                            self.current_state = NotEnoughResources(
    ...
    def has_enough_resources(self):
        ...
        if self.is_city:
            city_cost = {
                ResourceTile.MOUNTAIN.generate_resource().name(): 3,
                ResourceTile.PASTURE.generate_resource().name(): 2,
            }

            for resource, amount in city_cost.items():
                if (
                    resource not in self.player.resources
                    or self.player.resources[resource] < amount
                ):
                    return False
            return True
        else:
            settlement_cost = {
                ResourceTile.HILLS.generate_resource().name(): 1,
            ...
            for resource, quantity in settlement_cost.items():
                if (
                    resource not in self.player.resources
                    or self.player.resources[resource] < quantity
                ):
                    return False
            return True
...
class RoadBuildState:
                ...
                for node_button in node_buttons:
                    if node_button.is_clicked(mouse_pos):
                        if not self.building_road:
                            ...
                            if self.has_enough_resources():
                                self.player.build_road(
                                    self.node1,
                                    self.node2,
                                    is_special_round=False,
                                )
                                built_roads.append(
                                    (self.node1, self.node2, self.player)
                                )
                                game_log.append(
                                    f"{self.player.name} built a road!"
                                )
                                self.current_state = MainGameState(self.player)
                            else:
                                self.current_state = NotEnoughResources(
    ...
    def draw(self, screen):
        ...
        """
        prompt_text = WORD_FONT.render(
            "click two adjacent nodes to build a road!", True, BLACK, RED
        )
...
class ChooseTradePartner:
    ...
    def __init__(self, player):
        ...
        self.back_button_text = self.font.render(
            "Back Button", True, BLACK, RED
        )
        ...
        for player in players:
            if player.name != self.player.name:
                player_name_text = self.font.render(
                    player.name, True, WHITE, player.color
                )
                player_rect = player_name_text.get_rect(
                    center=(x_offset, DISPLAY_HEIGHT // 2)
                )
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                for player in self.available_players:
                    if player[0].collidepoint(mouse_pos):
                        ...
                        self.current_state = ChooseResources(
                            self.player, self.trade_partner
                        )
...
class AcceptTradeState:
    ...
    def __init__(
        self, player, trade_partner, player_resources, trade_partner_resources
    ):
        """Constructor class"""
        self.player = player
        self.trade_partner = trade_partner
    ...
    def draw(self, screen):
        ...
        prompt_text = BIG_FONT.render(
            f"{self.trade_partner.name}: Accept or decline this offer",
            True,
            BLACK,
        )
        ...
        screen.blit(
            self.accept_text,
            (self.accept_rect.x + 10, self.accept_rect.y + 10),
        )

        pygame.draw.rect(screen, BLACK, self.decline_rect)
        screen.blit(
            self.decline_text,
            (self.decline_rect.x + 10, self.decline_rect.y + 10),
        )
                ...
                if node_button.is_clicked(event.pos):
                    if self.has_enough_resources:
                        built_settlements.append(
                            (
                                (
                                    node_button.x_pos - 20,
                                    node_button.y_pos - 30,
                                ),
                                self.player,
                            )
                        )
                        game_log.append(
                            f"{self.player.name} built settlement!"
                        )
                        self.current_state = MainGameState(
                            self.player, self.current_state
                        )
                    else:
                        game_log.append("Not enough Resources")
                        self.current_state = MainGameState(
...
class NotEnoughResources:
    ...
    def __init__(self, player):
        ...
        self.message_text = self.font.render(
            "Not enough resources!", True, WHITE, RED
        )
...
class EndMenu:
    ...
    def draw(self, screen):
        ...
        for player in self.players:
            text_surface = WORD_FONT.render(
                f"{player.name}: {player.victory_points} VP", True, BLACK
            )
            screen.blit(text_surface, (400, y))
            y += 30

        winner_text_surface = WORD_FONT.render(
            f"Congrats Nerd! {self.players[0].name}"
        )
